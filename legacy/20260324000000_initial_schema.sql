-- =============================================================================
-- SaaS Base
-- Initial Schema — Single Shared Schema Multitenant
-- =============================================================================
...
-- 3) User profiles (1:1 with auth.users)
...
-- 9) Work order enums
CREATE TYPE work_order_status   AS ENUM ('draft','scheduled','in_progress','completed','canceled');
...
-- 14) Events / Activity feed
CREATE TABLE IF NOT EXISTS events (
  id            uuid        PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id     uuid        NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  related_type  text        NOT NULL, -- 'work_order','elevator','inspection','site'
  related_id    uuid        NOT NULL,
  event_type    text        NOT NULL, -- 'status_change','comment','assignment','inspection_result'
  payload       jsonb,
  created_by    uuid,
  created_at    timestamptz NOT NULL DEFAULT now()
);

-- 15) Attachments (Supabase Storage references)
...
-- =============================================================================
-- HELPER FUNCTION — get tenant IDs for a user (fallback when no JWT claim)
-- =============================================================================
...
-- =============================================================================
-- RLS POLICIES
-- Convention: JWT claim 'role' and 'tenant_id' are used for fast policy checks.
-- If JWT doesn't include tenant_id, policies fall back to user_tenants lookup.
-- =============================================================================

-- SITES
CREATE POLICY "sites_tenant_read" ON sites FOR SELECT TO authenticated USING (
...
CREATE POLICY "sites_tenant_insert" ON sites FOR INSERT TO authenticated WITH CHECK (
  tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
...
CREATE POLICY "sites_tenant_update" ON sites FOR UPDATE TO authenticated USING (
  tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
  OR EXISTS (SELECT 1 FROM user_tenants ut WHERE ut.user_id = (SELECT auth.uid()) AND ut.tenant_id = sites.tenant_id AND ut.role IN ('tenant_admin','owner'))
  OR (auth.jwt() ->> 'role') = 'superadmin'
);
CREATE POLICY "sites_tenant_delete" ON sites FOR DELETE TO authenticated USING (
...
-- ELEVATORS
CREATE POLICY "elevators_read" ON elevators FOR SELECT TO authenticated USING (
  tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
  OR EXISTS (SELECT 1 FROM user_tenants WHERE user_id = (SELECT auth.uid()) AND tenant_id = elevators.tenant_id)
  OR (auth.jwt() ->> 'role') = 'superadmin'
);
CREATE POLICY "elevators_insert" ON elevators FOR INSERT TO authenticated WITH CHECK (
  tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
...
CREATE POLICY "elevators_update" ON elevators FOR UPDATE TO authenticated
  USING (
...
CREATE POLICY "elevators_delete" ON elevators FOR DELETE TO authenticated USING (
...
CREATE POLICY "work_orders_update" ON work_orders FOR UPDATE TO authenticated
  USING (
    ...
    OR EXISTS (SELECT 1 FROM work_order_assignments wa WHERE wa.work_order_id = work_orders.id AND wa.user_id = (SELECT auth.uid()))
    OR (auth.jwt() ->> 'role') = 'superadmin'
  )
  WITH CHECK (
    work_orders.tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
...
CREATE POLICY "wo_assignments_select" ON work_order_assignments FOR SELECT TO authenticated USING (
  tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
  OR EXISTS (SELECT 1 FROM user_tenants WHERE user_id = (SELECT auth.uid()) AND tenant_id = work_order_assignments.tenant_id)
  OR (auth.jwt() ->> 'role') = 'superadmin'
);
CREATE POLICY "wo_assignments_insert" ON work_order_assignments FOR INSERT TO authenticated WITH CHECK (
...
CREATE POLICY "wo_assignments_update" ON work_order_assignments FOR UPDATE TO authenticated USING (
  work_order_assignments.user_id = (SELECT auth.uid())
  OR EXISTS (SELECT 1 FROM user_tenants ut WHERE ut.user_id = (SELECT auth.uid()) AND ut.tenant_id = work_order_assignments.tenant_id AND ut.role = 'tenant_admin')
  OR (auth.jwt() ->> 'role') = 'superadmin'
);
CREATE POLICY "wo_assignments_delete" ON work_order_assignments FOR DELETE TO authenticated USING (
...
-- INSPECTIONS
CREATE POLICY "inspections_select" ON inspections FOR SELECT TO authenticated USING (
  tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
  OR EXISTS (SELECT 1 FROM user_tenants WHERE user_id = (SELECT auth.uid()) AND tenant_id = inspections.tenant_id)
  OR (auth.jwt() ->> 'role') = 'superadmin'
);
CREATE POLICY "inspections_insert" ON inspections FOR INSERT TO authenticated WITH CHECK (
...
CREATE POLICY "inspections_update" ON inspections FOR UPDATE TO authenticated USING (
...
CREATE POLICY "attachments_insert" ON attachments FOR INSERT TO authenticated WITH CHECK (
...
-- AUDIT LOGS
CREATE POLICY "audit_insert" ON audit_logs FOR INSERT TO authenticated WITH CHECK (
