...
function buildJsonSchemaExample(
  ...
  const rolesJson = `[
    { "key": "${roleA}", "label": "${labelA}", "entity_type": "employee", "for_signing": true },
    { "key": "${roleB}", "label": "${labelB}", "entity_type": "employee", "for_signing": true }
  ]`
  const variablesJson = `[
...
export function buildAiTemplatePrompt(opts: PromptGeneratorOptions): string {
  const lines: string[] = []
  const useCase = opts.useCaseHint?.trim() || '[DESCRIPCIÓ DEL DOCUMENT, ex: Acord de confidencialitat (NDA) per a empleats]'
  ...
  if (cfg) {
    ...
    if (cfg.preferPathBasedIdentity) {
