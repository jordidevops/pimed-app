...
export function CatalogPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-5">
      ...
      {/* Tabs */}
      <div className="flex gap-1 rounded-lg border border-border p-0.5 bg-background w-fit">
        {(['service', 'product'] as KindTab[]).map((tab) => (
          <button
            key={tab}
            type="button"
          ...
          >
            {tab === 'service'
              ? t('catalog.tabs.services', 'Serveis')
              : t('catalog.tabs.products', 'Productes')}
          </button>
        ))}
      </div>
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">
                    {item.unit
                      ? t(`catalog.units.${item.unit}`, item.unit)
                      : '—'}
                  </td>
