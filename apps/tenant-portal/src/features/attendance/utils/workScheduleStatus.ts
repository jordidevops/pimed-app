...
import type { CurrentPunchStatus } from './punchProfileUi'
...
function isWorkingPresence(status: CurrentPunchStatus): boolean {
  return status === 'working' || status === 'traveling'
}

function isOutPresence(status: CurrentPunchStatus): boolean {
...
export function computeWorkScheduleStatus(
  ...
  if (schedule.holidayName || schedule.laborDayType === 'holiday' || schedule.dayType === 'holiday') {
    return {
      kind: 'info',
