/**
 * Work schedule vs punch presence evaluation (shared server + client parity).
 * Keep in sync with client workScheduleStatus.ts (tenant-portal + public-portal).
 */
  ...
  if (phase.phase === "break") {
    ...
    if (working) {
      return {
        ...
        descriptionDefault:
          "No has fitxat la sortida del torn de matí. Si ja has acabat, fitxa sortida.",
      };
    }
