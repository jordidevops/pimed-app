...
export class TwilioWhatsAppAdapter implements ChannelAdapter {
  ...
  async send(ctx: AdapterContext) {
    ...
    const from = creds.whatsapp_from_number ?? creds.messaging_service_sid;
    if (!from) throw new Error("TWILIO_WHATSAPP_FROM_NOT_CONFIGURED");

    const to = ctx.address.phoneE164.startsWith("whatsapp:")
      ? ctx.address.phoneE164
      : `whatsapp:${ctx.address.phoneE164}`;
    ...
    if (!res.ok) {
      ...
      throw new Error(`Twilio WhatsApp ${res.status}: ${text.slice(0, 500)}`);
    }

    const json = await res.json() as { sid?: string };
    return { provider: "twilio_whatsapp", providerMessageId: json.sid };
  }
