/**
 * process-playbook-queue — Worker PGMQ per Playbook Awareness (audit → tasques sistema).
 */
...
const FEATURE = "process-playbook-queue";
const QUEUE_NAME = "playbook_dispatch_queue";
const BATCH_SIZE = 50;
...
function createDispatchHandler(db: ReturnType<typeof createAdminClient>): TaskHandler {
  return async (payload: TaskPayload) => {
    const auditLogId = String(payload.audit_log_id ?? "");
    const playbookId = String(payload.playbook_id ?? "");

    if (!auditLogId || !playbookId) {
      log("warn", FEATURE, "Invalid playbook payload", { extra: { payload } });
      return { success: false };
    }

    const { data, error } = await db.rpc("process_playbook_dispatch", {
      p_audit_log_id: auditLogId,
      p_playbook_id: playbookId,
    });

    if (error) {
      log("error", FEATURE, "process_playbook_dispatch failed", {
        extra: { error: error.message, auditLogId, playbookId },
      });
...
Deno.serve(async (req) => {
  ...
  const runner = new QueueRunner({
    ...
    handlers: {
      dispatch_playbook: createDispatchHandler(db),
    },
    db,
    maxAttempts: 5,
    batchSize,
