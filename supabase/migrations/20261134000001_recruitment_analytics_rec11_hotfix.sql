-- =============================================================================
-- REC-11 hotfix — feature flag gate + interview locale + active_postings KPI
-- =============================================================================
-- Fixes:
-- 1. Require recruitment_enabled (SECURITY DEFINER bypasses RLS otherwise).
-- 2. Funnel "interview": also match stage name ILIKE '%interview%'.
-- 3. active_postings is not a person-cohort: show raw count (no k-suppress).
-- =============================================================================
...
BEGIN
  ...
  metrics AS (
    SELECT
      ...
      count(*) FILTER (
        WHERE has_interview
           OR stage_name ILIKE '%entrevista%'
           OR stage_name ILIKE '%interview%'
      )::bigint AS interview,
      count(*) FILTER (
  ...
  SELECT
    jsonb_build_object(
      ...
      'kpis', jsonb_build_object(
        ...
        -- Not a person cohort: always expose the posting inventory count
        'active_postings', jsonb_build_object(
          'value', p.active_postings,
          'suppressed', false
        ),
...
COMMENT ON FUNCTION api.get_recruitment_analytics(date, date, uuid, uuid, uuid) IS
  'REC-11: aggregated recruitment analytics with server-side k-anonymity. Requires recruitment_enabled + recruitment.view. No PII.';
