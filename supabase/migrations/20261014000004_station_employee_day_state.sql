-- Estació kiosk: estat del dia + següent acció in/out per empleat.
...
DECLARE
  ...
  v_work_date date := COALESCE(p_work_date, (now() AT TIME ZONE 'Europe/Madrid')::date);
  v_punch     record;
  v_next      text;
BEGIN
  FOR v_punch IN
...
REVOKE ALL ON FUNCTION data.compute_employee_punch_day_state(uuid, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION data.compute_employee_punch_day_state(uuid, date) TO service_role;
...
-- Validació explícita abans de la seqüència genèrica (missatges kiosk).
