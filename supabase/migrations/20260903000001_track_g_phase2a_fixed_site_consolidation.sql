-- Track G Phase 2a: fixed_site consolidation motor (cortesia + rounding + buckets)
-- PRD: docs/plans/checkin/plan-effective-work-time.md §4, §5.1, §11.4

-- =============================================================================
-- 1. Schema — bucket columns
-- =============================================================================
...
COMMENT ON COLUMN data.time_daily_summaries.effective_minutes IS
  'Track G: temps efectiu consolidat (fixed_site: intersecció horari + cortesia + arrodoniment).';
COMMENT ON COLUMN data.time_daily_summaries.paid_minutes IS
  'Track G: temps remunerable segons política de conveni.';
COMMENT ON COLUMN data.time_daily_summaries.consolidation_meta IS
  'Track G: metadades de consolidació (policy_version, rules_applied, skipped reason).';
...
VALUES
  (
    'attendance_effective_time_enabled',
    'tenant',
    'settings.manage',
    false,
    true,
    'Activa el motor de temps efectiu (Track G). Perfil resolt selecciona branca: fixed_site (G2a) o mobile (G2b).'
  )
...
-- =============================================================================
-- 3. Rounding helpers (§4.6)
-- =============================================================================

CREATE OR REPLACE FUNCTION data.truncate_to_quarter_local(
  p_ts timestamptz,
  p_tz text
)
RETURNS timestamptz
...
DECLARE
  ...
  v_time     time;
BEGIN
  v_local := p_ts AT TIME ZONE p_tz;
  v_date := v_local::date;
  v_minutes := EXTRACT(HOUR FROM v_local)::int * 60 + EXTRACT(MINUTE FROM v_local)::int;
  v_minutes := (v_minutes / 15) * 15;
  v_time := make_time(v_minutes / 60, v_minutes % 60, 0);
  RETURN (v_date + v_time) AT TIME ZONE p_tz;
END;
$$;

CREATE OR REPLACE FUNCTION data.ceil_to_quarter_local(
  p_ts timestamptz,
  p_tz text
)
RETURNS timestamptz
...
DECLARE
  ...
  v_ceil  timestamptz;
BEGIN
  v_floor := data.truncate_to_quarter_local(p_ts, p_tz);
  IF v_floor = p_ts THEN
    RETURN p_ts;
  END IF;
  v_ceil := v_floor + interval '15 minutes';
  RETURN v_ceil;
END;
$$;

CREATE OR REPLACE FUNCTION data.adjust_punch_for_consolidation(
  p_occurred_at       timestamptz,
  p_punch_type        text,
  p_expected_boundary timestamptz,
  p_policy            jsonb,
  p_tz                text
)
RETURNS timestamptz
...
DECLARE
  v_mode      text;
  v_direction text;
  v_rounded   timestamptz;
BEGIN
  IF p_occurred_at IS NULL THEN
    RETURN NULL;
  END IF;

  v_mode := COALESCE(p_policy->'rounding'->>'mode', 'real_minute');
  v_direction := COALESCE(p_policy->'rounding'->>'direction', 'favor_employee');

  IF v_mode = 'real_minute' THEN
    RETURN p_occurred_at;
  END IF;

  IF p_punch_type IN ('in', 'day_start') THEN
    -- Invariant: mai posterior al fitxatge real (§4.6)
    IF p_occurred_at > p_expected_boundary THEN
      IF v_direction = 'favor_employer' THEN
        RETURN p_occurred_at;
      END IF;
      v_rounded := data.truncate_to_quarter_local(p_occurred_at, p_tz);
      RETURN LEAST(v_rounded, p_occurred_at);
    END IF;

    v_rounded := data.truncate_to_quarter_local(p_occurred_at, p_tz);
    RETURN LEAST(v_rounded, p_occurred_at);

  ELSIF p_punch_type IN ('out', 'day_end') THEN
    -- Invariant: mai anterior al fitxatge real (§4.6)
    IF v_direction = 'favor_employer' THEN
      RETURN p_occurred_at;
    END IF;
    v_rounded := data.ceil_to_quarter_local(p_occurred_at, p_tz);
    RETURN GREATEST(v_rounded, p_occurred_at);
  END IF;
...
REVOKE ALL ON FUNCTION data.truncate_to_quarter_local(timestamptz, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION data.ceil_to_quarter_local(timestamptz, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION data.adjust_punch_for_consolidation(timestamptz, text, timestamptz, jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION data.truncate_to_quarter_local(timestamptz, text) TO service_role;
GRANT EXECUTE ON FUNCTION data.ceil_to_quarter_local(timestamptz, text) TO service_role;
GRANT EXECUTE ON FUNCTION data.adjust_punch_for_consolidation(timestamptz, text, timestamptz, jsonb, text) TO service_role;

-- =============================================================================
-- 4. Courtesy bounds for WORK (fixed_site)
-- =============================================================================
...
BEGIN
  ...
  -- IN courtesy
  IF p_in_at <= v_expected_start THEN
    v_early_min := GREATEST(0, (EXTRACT(EPOCH FROM (v_expected_start - p_in_at)) / 60)::int);
    IF v_early_min <= COALESCE((v_courtesy->>'early_arrival_minutes')::int, 15) THEN
      v_eff_in := v_expected_start;
    ELSE
      IF v_overflow_early = 'needs_review' THEN
        v_anomalies := array_append(v_anomalies, 'EFFECTIVE_OVERFLOW_EARLY');
      END IF;
      v_eff_in := v_expected_start;
    END IF;
  ELSE
    v_late_min := GREATEST(0, (EXTRACT(EPOCH FROM (p_in_at - v_expected_start)) / 60)::int);
  ...
  -- OUT courtesy
  IF p_out_at >= v_expected_end THEN
    v_late_out_min := GREATEST(0, (EXTRACT(EPOCH FROM (p_out_at - v_expected_end)) / 60)::int);
    IF v_late_out_min <= COALESCE((v_courtesy->>'late_departure_minutes')::int, 15) THEN
      v_eff_out := v_expected_end;
    ELSE
      v_eff_out := p_out_at;
      IF v_late_out_min > COALESCE((v_courtesy->>'late_departure_minutes')::int, 15) THEN
    ...
    END IF;
  ELSE
    v_early_out_min := GREATEST(0, (EXTRACT(EPOCH FROM (v_expected_end - p_out_at)) / 60)::int);
...
REVOKE ALL ON FUNCTION data.apply_courtesy_work_bounds(timestamptz, timestamptz, date, jsonb, jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION data.apply_courtesy_work_bounds(timestamptz, timestamptz, date, jsonb, jsonb, text) TO service_role;

-- =============================================================================
-- 5. consolidate_day_buckets — fixed_site branch
-- =============================================================================
...
BEGIN
  ...
  -- Pas 2 OT (§4.2): excés de OUT ajustat respecte expected_end
  ...
  v_meta := jsonb_build_object(
    ...
    'rules_applied', jsonb_build_array('courtesy', 'rounding', 'schedule_intersection'),
    'adjusted_in', v_adj_in,
    'adjusted_out', v_adj_out,
    'buckets', jsonb_build_object(
      'presence', GREATEST(0, v_gross_min - v_break_unpaid_min),
  ...
  RETURN jsonb_build_object(
    'skipped', false,
    'work_profile', v_profile,
    'presence_minutes', GREATEST(0, v_gross_min - v_break_unpaid_min),
...
-- =============================================================================
-- 6. Patch recompute_attendance_worker — call consolidate for fixed_site
-- =============================================================================
...
-- =============================================================================
-- 7. api.time_daily_summaries — expose bucket columns
-- =============================================================================

DROP VIEW IF EXISTS api.time_daily_summaries CASCADE;

CREATE VIEW api.time_daily_summaries
  WITH (security_invoker = true) AS
  SELECT
    id, tenant_id, site_id, employee_id, work_date,
    day_type, expected_minutes, worked_minutes, break_minutes,
    overtime_minutes, absence_minutes, punch_count,
    presence_minutes, work_minutes, travel_minutes,
