-- LC-2 fix (generated): retention_status guards for share + staff resolve
CREATE OR REPLACE FUNCTION api.resolve_customer_portal_share_session(p_session_token_hash bytea, p_action text DEFAULT 'report_view'::text, p_ip_address inet DEFAULT NULL::inet, p_user_agent text DEFAULT NULL::text, p_request_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
