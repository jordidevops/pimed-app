-- Entity Timeline — pinning (Fase 2)

CREATE OR REPLACE FUNCTION api.pin_entity_comment(
  p_id      uuid,
...
BEGIN
  ...
  IF v_row.parent_id IS NOT NULL THEN
    RAISE EXCEPTION 'only_root_comments_can_be_pinned' USING ERRCODE = '22023';
  END IF;

  IF NOT data.can_edit_entity(
    auth.uid(), v_row.tenant_id, v_row.entity_type, v_row.entity_id, v_row.site_id
  ) THEN
...
GRANT EXECUTE ON FUNCTION api.pin_entity_comment(uuid, boolean) TO authenticated;

-- -----------------------------------------------------------------------------
-- get_entity_timeline — comentaris fixats sempre a dalt (primera pàgina)
-- -----------------------------------------------------------------------------
