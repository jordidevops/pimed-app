-- EX-01.1: QR atòmic — resolve = previsualització; consum del token dins record_station_time_punch.
...
DROP FUNCTION IF EXISTS api.record_station_time_punch(uuid, uuid, uuid, text, text, text, jsonb);
...
REVOKE ALL ON FUNCTION api.record_station_time_punch(uuid, uuid, uuid, text, text, text, jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.record_station_time_punch(uuid, uuid, uuid, text, text, text, jsonb, text) TO service_role;

COMMENT ON FUNCTION api.resolve_attendance_identity_token IS
  'Previsualitza identitat QR/barcode sense consumir el token (EX-01.1).';

COMMENT ON FUNCTION api.record_station_time_punch IS
  'Fitxatge estació; source=qr exigeix p_identity_token i consumeix el token dins la mateixa transacció.';
