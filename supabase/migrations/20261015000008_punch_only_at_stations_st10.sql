-- EX-02.1 / ST-10: punch_only_at_stations — bloqueig fitxatge mòbil/portal, consulta horari sí.

INSERT INTO data.settings_registry
  (setting_key, scope, required_permission, owner_only, is_active, description)
VALUES
  ('punch_only_at_stations', 'tenant', 'settings.manage', false, true,
   'Només permet fitxar des d''estacions o QR estació; el portal personal queda en mode consulta.')
...
VALUES ('defaults', '{"punch_only_at_stations": false}'::jsonb)
ON CONFLICT (module) DO UPDATE
  SET settings = data.system_settings.settings || EXCLUDED.settings,
      updated_at = now();

CREATE OR REPLACE FUNCTION data.is_punch_only_at_stations(
  p_tenant_id uuid,
...
BEGIN
  IF p_site_id IS NOT NULL THEN
    SELECT (s.settings->>'punch_only_at_stations')::boolean
      INTO v_site_val
    FROM data.sites s
    WHERE s.id = p_site_id;
    IF v_site_val IS NOT NULL THEN
      RETURN v_site_val;
    END IF;
  END IF;

  SELECT (t.settings->>'punch_only_at_stations')::boolean
    INTO v_tenant_val
  FROM data.tenants t
...
REVOKE ALL ON FUNCTION data.is_punch_only_at_stations(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION data.is_punch_only_at_stations(uuid, uuid) TO service_role;
...
BEGIN
  ...
  IF NOT data.is_punch_only_at_stations(p_tenant_id, p_site_id) THEN
    RETURN;
  END IF;

  IF p_source = 'mobile'
     AND auth.uid() IS NOT NULL
     AND data.jwt_has_permission(p_tenant_id, 'attendance.adjust', p_site_id) THEN
    RETURN;
  END IF;
...
REVOKE ALL ON FUNCTION data.assert_portal_mobile_punch_allowed(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION data.assert_portal_mobile_punch_allowed(uuid, uuid, text) TO service_role;

-- Patch record_time_punch: enforce ST-10 after settings resolution.
...
BEGIN
  ...
  PERFORM data.assert_portal_mobile_punch_allowed(
...
-- Expose policy flag to employee portal today payload.
...
BEGIN
  ...
  v_punch_only := data.is_punch_only_at_stations(v_emp.tenant_id, v_emp.site_id);
