-- CP: bulletin materials section (tenant default + draft/version + candidates + projection)

-- ---------------------------------------------------------------------------
-- 1. Tenant default + draft/version columns
-- ---------------------------------------------------------------------------
ALTER TABLE data.customer_portal_tenant_state
  ADD COLUMN IF NOT EXISTS bulletin_show_materials boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN data.customer_portal_tenant_state.bulletin_show_materials IS
  'Default: include materials section on customer bulletins (per-draft override may differ).';

ALTER TABLE data.customer_intervention_report_drafts
  ADD COLUMN IF NOT EXISTS show_materials boolean;

ALTER TABLE data.customer_intervention_report_versions
  ADD COLUMN IF NOT EXISTS show_materials boolean;

COMMENT ON COLUMN data.customer_intervention_report_drafts.show_materials IS
  'NULL = inherit tenant bulletin_show_materials.';
...
BEGIN
  ...
  v_row.bulletin_show_materials := true;
...
-- ---------------------------------------------------------------------------
-- 3. Safe projection allowlist (+ materials; still strip costs)
-- ---------------------------------------------------------------------------
...
AS $$
  SELECT jsonb_strip_nulls(jsonb_build_object(
    ...
    'materials', COALESCE(p_projection->'materials', '[]'::jsonb),
...
-- ---------------------------------------------------------------------------
-- 4. Candidates: include materials
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Public fields only: never unit_price_cents / is_billable
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', m.id,
      'name', m.name,
  ...
  RETURN jsonb_build_object(
    ...
    'tenant_show_materials', COALESCE(v_tstate.bulletin_show_materials, true)
  );
END;
$$;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'show_materials', COALESCE(v_tstate.bulletin_show_materials, true)
  );
END;
$$;
...
DROP FUNCTION IF EXISTS api.set_my_customer_portal_bulletin_content_defaults(boolean, boolean);
...
CREATE OR REPLACE FUNCTION api.set_my_customer_portal_bulletin_content_defaults(
  ...
  p_show_materials boolean DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    ...
    bulletin_show_materials = COALESCE(p_show_materials, bulletin_show_materials),
    updated_at = now()
  ...
  PERFORM data.log_audit_event_strict(
    ...
    jsonb_build_object(
      ...
      'show_materials', p_show_materials
    )
  );
...
REVOKE ALL ON FUNCTION api.set_my_customer_portal_bulletin_content_defaults(boolean, boolean, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.set_my_customer_portal_bulletin_content_defaults(boolean, boolean, boolean)
  TO authenticated;

-- ---------------------------------------------------------------------------
-- 6. Upsert draft (show_materials)
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS api.upsert_customer_intervention_report_draft(
...
CREATE OR REPLACE FUNCTION api.upsert_customer_intervention_report_draft(
  ...
  p_clear_show_materials boolean DEFAULT false
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_draft_id IS NOT NULL THEN
    ...
    SET
      ...
      END,
      show_materials = CASE
        WHEN p_clear_show_materials THEN NULL
        WHEN p_show_materials IS NOT NULL THEN p_show_materials
        ELSE show_materials
      END,
  ...
  INSERT INTO data.customer_intervention_report_drafts (
    ...
    show_checklists, show_tasks, show_materials, content_selection,
    ...
    CASE WHEN p_clear_show_materials THEN NULL ELSE p_show_materials END,
...
REVOKE ALL ON FUNCTION api.upsert_customer_intervention_report_draft(
  uuid, text, text, jsonb, jsonb, uuid, boolean, boolean, boolean, jsonb, boolean, boolean, boolean
) FROM PUBLIC;
...
-- ---------------------------------------------------------------------------
-- 7. Publish: freeze materials visibility
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  v_show_materials := COALESCE(v_draft.show_materials, v_tstate.bulletin_show_materials, true);
  ...
  END IF;
  IF NOT v_show_materials THEN
    v_proj := jsonb_set(v_proj, '{materials}', '[]'::jsonb, true);
  END IF;
  v_proj := jsonb_set(
    v_proj,
    '{visibility}',
    jsonb_build_object(
      ...
      'show_materials', v_show_materials
    ),
    true
  );
  ...
  INSERT INTO data.customer_intervention_report_versions (
    ...
    auth.uid(),
    v_show_cl, v_show_tasks, v_show_materials, COALESCE(v_draft.content_selection, '{}'::jsonb)
  )
...
-- ---------------------------------------------------------------------------
-- 8. Corrected draft inherits show_materials
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO data.customer_intervention_report_drafts (
    ...
    v_ver.show_checklists, v_ver.show_tasks, v_ver.show_materials,
    COALESCE(v_ver.content_selection, '{}'::jsonb),
    auth.uid(), auth.uid()
  )
  RETURNING id INTO v_draft;
