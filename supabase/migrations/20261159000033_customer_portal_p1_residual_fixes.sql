-- =============================================================================
-- P1 residual: staff resolve must not consume URL handoffs (stream/media path)
-- =============================================================================

DROP FUNCTION IF EXISTS api.exchange_customer_portal_staff_session(bytea, inet, text, uuid);
...
-- Pre-migration handoffs that were stamped exchanged_at without hash rotation:
-- revoke any still-valid session whose URL hash was never rotated (last_seen equals
-- exchanged_at from backfill and no subsequent opaque use is distinguishable).
-- Safer: revoke sessions created before this migration that never got a true consume
-- log entry. Handoffs with handoff_consumed logs are fine.
UPDATE data.customer_portal_staff_sessions s
SET revoked_at = now()
WHERE s.revoked_at IS NULL
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM data.customer_report_share_access_logs l
    WHERE l.session_id = s.id
      AND l.action = 'handoff_consumed'
  );
