-- EP-ACC-8a: RPC resum accés portal per tenant (hub)
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      kiosk.token_id AS kiosk_token_id,
      kiosk.first_accessed_at AS kiosk_first_accessed_at,
      kiosk.last_accessed_at AS kiosk_last_accessed_at,
      kiosk.created_at AS kiosk_created_at,
      kiosk.label AS kiosk_label,
      GREATEST(personal.last_accessed_at, kiosk.last_accessed_at) AS last_access_any
    FROM permitted p
    LEFT JOIN LATERAL (
      SELECT
        t.id AS token_id,
        t.pin_must_set,
        (t.pin_hash IS NOT NULL OR NOT COALESCE(t.pin_must_set, false)) AS pin_configured,
        t.first_accessed_at,
        t.last_accessed_at,
        t.created_at,
        t.label
      ...
      WHERE t.employee_id = p.employee_id
        ...
        AND COALESCE(t.shared_device, false) = false
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE t.employee_id = p.employee_id
        ...
        AND t.revoked_at IS NULL
        AND (t.expires_at IS NULL OR t.expires_at > now())
        AND COALESCE(t.shared_device, false) = true
      ORDER BY t.created_at DESC
      LIMIT 1
    ) kiosk ON true
  ),
  filtered AS (
    ...
    WHERE
      ...
      OR (
        v_portal_filter = 'kiosk_only'
        AND e.kiosk_token_id IS NOT NULL
        AND e.personal_token_id IS NULL
      )
      OR (
        v_portal_filter = 'pin_not_configured'
        AND e.personal_token_id IS NOT NULL
        AND COALESCE(e.personal_pin_must_set, false) = true
  ...
  paged AS (
    SELECT
      ...
      jsonb_build_object(
        'has_active', f.kiosk_token_id IS NOT NULL,
        'token_id', f.kiosk_token_id,
        'first_accessed_at', f.kiosk_first_accessed_at,
        'last_accessed_at', f.kiosk_last_accessed_at,
        'created_at', f.kiosk_created_at,
        'label', f.kiosk_label
      ) AS kiosk
    FROM filtered f
    ORDER BY
  ...
  SELECT jsonb_build_object(
    ...
    'without_personal_link', (
      SELECT count(*)::int
      FROM active_enriched ae
      WHERE ae.personal_token_id IS NULL
    ),
    'never_opened', (
      SELECT count(*)::int
      FROM active_enriched ae
      WHERE ae.personal_token_id IS NOT NULL
        AND ae.personal_first_accessed_at IS NULL
    ),
    'missing_document_id', (
      SELECT count(*)::int
      FROM permitted_active pa
      WHERE pa.missing_document_id
    ),
...
COMMENT ON FUNCTION api.list_employee_portal_access_overview IS
  'Resum accés portal per empleat (hub EP-ACC-8). Filtra per permís attendance.manage per site.';
