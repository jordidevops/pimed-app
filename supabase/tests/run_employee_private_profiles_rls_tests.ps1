...
$test = "employee_private_profiles_rls_tests.sql"
$path = Join-Path $root $test

Write-Host "Running EHR private profile RLS tests against container: $DbContainer" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0 -or $hasSqlError -or $hasRowFail -or $passCount -lt 8) {
  Write-Host "[FAIL] $test (pass rows=$passCount)" -ForegroundColor Red
  exit 1
}
