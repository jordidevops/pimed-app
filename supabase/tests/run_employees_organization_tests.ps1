...
$test = "employees_organization_tests.sql"
$path = Join-Path $root $test

Write-Host "Running EHR organization tests against container: $DbContainer" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0 -or $hasSqlError -or $hasRowFail -or $passCount -lt 9) {
