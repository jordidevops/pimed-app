...
$test = "ec_elm_source_contract_tests.sql"
$path = Join-Path $root $test

Write-Host "Running EC↔ELM source=contract tests against container: $DbContainer" -ForegroundColor Cyan
...
$passCount = ([regex]::Matches($outputText, '(?m)\|\s*PASS\s*\|')).Count

if ($LASTEXITCODE -ne 0 -or $hasSqlError -or $hasRowFail -or $passCount -lt 5) {
  Write-Host ('[FAIL] {0} (pass rows={1})' -f $test, $passCount) -ForegroundColor Red
  exit 1
}

Write-Host ('[PASS] {0}' -f $test) -ForegroundColor Green
exit 0
