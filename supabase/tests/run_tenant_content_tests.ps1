...
$path = Join-Path $root "tenant_content_tests.sql"

Write-Host "Running tenant content SQL tests against: $DbContainer" -ForegroundColor Cyan
...
$testsPassed = $outputText -match 'TCMS F2: all tests passed'

if ($LASTEXITCODE -ne 0 -or $hasSqlErrorText -or -not $testsPassed) {
  throw "tenant_content_tests.sql failed."
}

Write-Host "tenant content SQL tests passed." -ForegroundColor Green
