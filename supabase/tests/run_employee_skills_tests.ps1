...
$test = "employee_skills_tests.sql"
$path = Join-Path $root $test

Write-Host "Running M-EHR-06 employee skills tests against container: $DbContainer" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0 -or $hasSqlError -or $hasRowFail -or $passCount -lt 6) {
