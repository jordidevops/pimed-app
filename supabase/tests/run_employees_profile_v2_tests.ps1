...
$test = "employees_profile_v2_tests.sql"
$path = Join-Path $root $test

Write-Host "Running EHR-1 profile tests against container: $DbContainer" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0 -or $hasSqlError -or $hasRowFail -or $passCount -lt 7) {
